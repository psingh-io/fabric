...
func testGenerateLocalMSP(t *testing.T, nodeOUs bool) {
	...
	// generate signing CA
	signCA, err := ca.NewCA(caDir, testCAOrg, testCAName, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode, 365*15*24*time.Hour, 365*24*time.Hour)
	assert.NoError(t, err, "Error generating CA")
	// generate TLS CA
	tlsCA, err := ca.NewCA(tlsCADir, testCAOrg, testCAName, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode, 365*15*24*time.Hour, 365*24*time.Hour)
	assert.NoError(t, err, "Error generating CA")
